# Calculate the True Cost of Credit Card Debt

# Calculate the True Cost of Credit Card Debt

Credit Cards! These are usually the first “loans” a person takes out, and the first monthly payments! Tens of thousands of young people dig themselves deep into credit card debt before they even realize it, so have fun with this payments calculator to see how much these bits of plastic REALLY cost!

If you have already used our **[Compound Interest Calculator](/content/index?url=savings-growth-and-compound-interest-calculator)**, try using the same interest rate you hoped to save at as the same amount your credit card company charges. Then double it to get a more realistic picture!

Once you have seen the impact that taking out loans can have at different interest rate and payment schemes, check out our **[Car Loan Calculator](/content/index?url=car-loan-calculator-2)** to see how you can budget a big purchase without losing your wallet!

# Credit Card Payoff Calculator

Enter up to 5 credit cards. Minimum payments are treated as a % of the *current* balance each month (no fixed dollar floor).

### Card 1

Remove

Current Balance (min 0, max 100,000)
Interest Rate (APR %) (min 0, max 36)
Minimum Payment (% of current balance) (min 0, max 50)

Minimum Payment (this month)

$200.00

Months to Payoff

117

Total Payments

$8,017.12

Minimum payment is too low to cover monthly interest. Balance will never pay off.

Add Card (up to 5)
 Reset to default

## Totals

Total Initial Balance

$5,000.00

Total Payments

$8,017.12

Payoff Time:

117 months

9.8 years

## Outstanding Debt Over Time

### Card #

Remove

Current Balance (min 0, max 100,000)
Interest Rate (APR %) (min 0, max 36)
Minimum Payment (% of current balance) (min 0, max 50)

Minimum Payment (this month)

$0.00

Months to Payoff

0

Total Payments

$0.00

Minimum payment is too low to cover monthly interest. Balance will never pay off.

**Set all credit cards to 0 except for the first one. How much will it cost you in interest for $1000 on your credit card at 25% interest rate and a minimum payment of 10%, making the minimum payments?**

|  |  |
| --- | --- |
|  | Less Than $100 |
|  | Between $100 and $200 |
|  | Between $200 and $300 |
|  | More than $300 |

**For the same values: $1000 on your credit card at 25% interest rate and a minimum payment of 10%. How long will it take to pay off your credit card by making minimum payments?**

|  |  |
| --- | --- |
|  | Less than a year |
|  | Between 1 and 2 years |
|  | Between 2 and 4 years |
|  | Never |

**For $10,000 on your credit card at 15% interest and a minimum payment of 5%, how long will it take to pay off your credit card by making the minimum payments?**

|  |  |
| --- | --- |
|  | Less than 2 years |
|  | Between 2 and 5 years |
|  | Between 5 and 7 years |
|  | More than 7 years |

**If you have $1000 on your first credit card at 25% interest rate with a minimum payment of 10%, and also $2000 on your second credit card at 10% interest rate with a minimum payment of 10% how long will it take to payoff both your credit cards if you start paying both at the same time, making the minimum payments?**

|  |  |
| --- | --- |
|  | Less than 15 months |
|  | Between 15 and 30 months |
|  | Between 30 and 40 months |
|  | More than 40 months |

**For the values in the previous question, if you could only pay off one credit card right now and continue to make the minimum payments on the second, which should you choose?**

|  |  |
| --- | --- |
|  | #1 because the overall interest paid is higher than the second card |
|  | #2 because it has a higher balance |
|  | #1 because it has a lower balance |
|  | #2 because the overall interest paid is higher than the first card |

Submit
Please complete all the questions


---
## Transcript (best effort)

Calculate the True Cost of Credit Card Debt
Credit Cards! These are usually the first “loans” a person takes out, and the first monthly payments! Tens of thousands of young people dig themselves deep into credit card debt before they even realize it, so have fun with this payments calculator to see how much these bits of plastic REALLY cost!
If you have already used our
Compound Interest Calculator
, try using the same interest rate you hoped to save at as the same amount your credit card company charges. Then double it to get a more realistic picture!
Once you have seen the impact that taking out loans can have at different interest rate and payment schemes, check out our
Car Loan Calculator
to see how you can budget a big purchase without losing your wallet!
Credit Card Payoff Calculator
Enter up to 5 credit cards. Minimum payments are treated as a % of the
current
balance each month (no fixed dollar floor).
Card
1
Remove
Current Balance
(min 0, max 100,000)
Interest Rate (APR %)
(min 0, max 36)
Minimum Payment (% of current balance)
(min 0, max 50)
Minimum Payment (this month)
$200.00
Months to Payoff
117
Total Payments
$8,017.12
Minimum payment is too low to cover monthly interest. Balance will never pay off.
Add Card (up to 5)
Reset to default
Totals
Total Initial Balance
$5,000.00
Total Payments
$8,017.12
Payoff Time:
117 months
9.8 years
Outstanding Debt Over Time
