'use client';

import PopQuiz from '@/components/quiz/PopQuiz';

const chapter9ExamQuestions = [
  {
    id: 1,
    question: 'Which of the following is NOT a way that options can earn you money?',
    options: [
      'When the market moves in the direction you expect, trading options will amplify your profits.',
      'If the market price rises above the "limit price", the option converts to a market order and executes.',
      'Protective put options help you protect your profits in stocks you currently own and have achieved gains.',
      'Writing (selling) options can add income to your portfolio in a flat or stagnant market environment.',
    ],
    correctAnswer: 1,
  },
  {
    id: 2,
    question: 'How are options used in real estate transactions?',
    options: [
      'The option to remodel portions of an older home',
      'The option to sell back the property to the previous owner',
      'The option to sell the property on the open market',
      'The option to purchase additional neighboring acreage at a certain price within a certain number of years',
    ],
    correctAnswer: 3,
  },
  {
    id: 3,
    question: 'The buyer of a put option thinks what will happen in the near future?',
    options: [
      'There is high risk in the stock price',
      'The price of the security will rise',
      'The price of the security will fall',
      'The volume of shares traded will increase',
    ],
    correctAnswer: 2,
  },
  {
    id: 4,
    question: 'Which of the following is NOT a reason why an investor will buy a put option?',
    options: [
      'To speculate that the price will soon rise',
      'To speculate that the price will fall soon',
      'As protection against potential losses',
      'As an arbitrage position',
    ],
    correctAnswer: 0,
  },
  {
    id: 5,
    question: 'How do larger companies use options?',
    options: [
      'As an incentive for employees to stay with the company for a long time and help the company increase in value.',
      'As a recruitment tool to get the best talent',
      'As a competitive advantage over their direct competitors',
      'As a tax credit to diminish their net profits',
    ],
    correctAnswer: 0,
  },
  {
    id: 6,
    question: 'True or False - All companies have stock options?',
    options: ['TRUE', 'FALSE'],
    correctAnswer: 1,
  },
  {
    id: 7,
    question: 'What are the most common intervals of stock option prices?',
    options: ['1', '5', '25', '0.5'],
    correctAnswer: 1,
  },
  {
    id: 8,
    question: "If the current stock price is $55, and you're looking at a $60 call option for the next month, what does it mean if the contract price is $0.75?",
    options: [
      'A lot of people expect the price to have a massive breakout soon.',
      'A lot of people expect the price to drop drastically',
      'Most people expect the price to stay below $60',
      'Most people expect the price to not move at all',
    ],
    correctAnswer: 2,
  },
  {
    id: 9,
    question: 'Which of the following functions of an option price is not easily agreed upon by traders?',
    options: ['Stock price', 'Volatility', 'Strike Price', 'Days to expiration'],
    correctAnswer: 1,
  },
  {
    id: 10,
    question: 'What is a covered call?',
    options: [
      'When you own the underlying stock and write the call.',
      'When you buy the right, but not the obligation to buy an option contract.',
      'When you buy the right, but not the obligation to sell an option contract.',
      'When you do not own the underlying stock and write the call.',
    ],
    correctAnswer: 0,
  },
  {
    id: 11,
    question: 'What happens on the expiration date of an option contract?',
    options: [
      'The option automatically renews',
      'The option goes down in value',
      'The option is worthless if not exercised already',
      'Investors receive the face value of the coupon',
    ],
    correctAnswer: 2,
  },
  {
    id: 12,
    question: 'What factor makes it easier for the sellers or writers of options to "win"?',
    options: [
      'The usually are more knowledgeable about how the stock market works',
      'They own the underlying security',
      'Each day that goes by the odds of a price movement become less and less',
      "Implied volatility is usually on the seller's side",
    ],
    correctAnswer: 2,
  },
  {
    id: 13,
    question: 'If an option contract is trading at $5.45, what is the premium?',
    options: ['5.45', '45', '545', '450'],
    correctAnswer: 2,
  },
  {
    id: 14,
    question: 'For investors, what does high volatility usually signify?',
    options: [
      'Risk and uncertainty',
      'Implied Volatility',
      'Prices are going up',
      'Prices are going down',
    ],
    correctAnswer: 0,
  },
  {
    id: 15,
    question: 'True or False - High implied volatility affects the seller more than the buyer?',
    options: ['FALSE', 'TRUE'],
    correctAnswer: 0,
  },
  {
    id: 16,
    question: 'The Black-Scholes model helps option traders in what major way?',
    options: [
      'It calculates the correct strike price.',
      'It calculates the annual rate of return.',
      'It calculates the implied volatility.',
      'It calculates the expected price of the option at expiration date.',
    ],
    correctAnswer: 2,
  },
  {
    id: 17,
    question: 'What does it mean when more investors are buying calls than puts?',
    options: [
      'The option is on sale',
      'They are hedging against inflation',
      'Market sentiment is bullish',
      'Market sentiment is bearish',
    ],
    correctAnswer: 2,
  },
  {
    id: 18,
    question: "There are many more put than call options for a stock you're looking at, what should you do?",
    options: [
      'Sell your holdings in that stock',
      'Double down, and buy more shares',
      'Do nothing',
      'Analyze whether the put interest is legitimate',
    ],
    correctAnswer: 3,
  },
  {
    id: 19,
    question: 'True or False - The historical average for investors buying puts or calls is about the same for both.',
    options: ['TRUE', 'FALSE'],
    correctAnswer: 1,
  },
];

export default function Chapter9Exam(): JSX.Element {
  return (
    <article className="prose prose-lg max-w-none">
      <h1 className="text-3xl font-bold mb-4">Chapter 9 Exam</h1>
      <p className="my-3 leading-relaxed">
        Complete the following exam to test your knowledge of Chapter 9 concepts on options trading, puts, calls, and volatility.
      </p>

      <div className="not-prose mt-8">
        <PopQuiz
          lessonId="chapter-9-exam"
          questions={chapter9ExamQuestions}
          title="Chapter 9 Exam"
          timePerQuestion={60}
        />
      </div>
    </article>
  );
}
